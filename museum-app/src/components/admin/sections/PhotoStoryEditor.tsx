...
import { createEmptyStory, mediaRefFromUrl } from "@/lib/section-content";
    ...
    <div className="admin-template-form">
      {stories.length === 0 ? (
        <div className="admin-empty admin-empty--friendly">
          ...
          <p className="admin-empty__sub">Каждый сюжет — медиа с заголовком и текстом. На экране они чередуются слева и справа.</p>
        ...
        stories.map((story, index) => {
          const thumb = getMediaThumbnailUrl(story.media ?? mediaRefFromUrl(""));
          return (
            <article key={story.id} className="admin-story-card">
              ...
              <div className="admin-story-card__layout">
                <div className="admin-story-card__photo">
                  <MediaUploadZone
                    value={story.media}
                    onChange={(media) => update(story.id, { media })}
                  />
                  {story.media?.url ? (
                    <ImageSizePicker
                      compact
              ...
              {story.media?.url && story.title && thumb ? (
